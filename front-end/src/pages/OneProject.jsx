...
export default function OneProject(props) {
    ...
    return dataOneProject.length == 0 ? (
        <Loader />
    ) : (
        <div className="oneProject">
            ...
            {actionDeleteButton == 0 ? (
                <button
                    onClick={() =>
                        setActionDeleteButton(actionDeleteButton + 1)
                    }
                >
                    Supprimer le projet
                </button>
            ) : null}
            {actionDeleteButton == 1 ? (
                <p>"Êtes-vous vraiment sur de vouloir supprimer ce projet ?"</p>
            ) : null}
            {actionDeleteButton == 1 ? (
                <p>
                    "Cette action entrainera la suppression de toutes les
                    données du projet"
                </p>
            ) : null}
            {actionDeleteButton == 1 ? (
                <p>
                    "Si vous êtes sur cliquez une nouvelle fois sur le boutton"
                </p>
            ) : null}
            {actionDeleteButton == 1 ? (
                <button
            ...
            <div>
                <p>Description</p>
                <p>
                    {dataOneProject.description == undefined ? (
                        <Loader />
                    ) : (
                        dataOneProject.description
                    )}
                </p>
            </div>
            <div>
            ...
            {dataOneProject.task?.map((eachTask) => (
                <Link to={`/onetask/${eachTask.taskId}`} key={eachTask.taskId}>
